      ...
      customTitle: Row(
        ...
        children: [
          ...
          IconButton(
            ...
            tooltip: 'Crear nuevo beneficiario',
            onPressed: () {
              showGeneralDialog(
                context: context,
                barrierLabel: 'Nuevo beneficiario',
                barrierDismissible: true,
                barrierColor: Colors.black.withOpacity(0.5),
                transitionDuration: const Duration(milliseconds: 300),
                pageBuilder: (BuildContext context, Animation<double> animation, Animation<double> secondaryAnimation) {
                  return const NewBeneficiaryPanel();
                },
            ...
            child: Padding(
              ...
              child: Obx(() {
                if (controller.beneficiaries.isEmpty) {
                  return const Center(child: Text('No hay beneficiarios'));
                }
                return ListView.builder(
                  itemCount: controller.beneficiaries.length,
                  itemBuilder: (context, index) {
                    final DocumentSnapshot beneficiary = controller.beneficiaries[index];
                    return BeneficiaryTile(
                      key: ValueKey(beneficiary.id),
                      beneficiary: beneficiary,
                      onDelete: () async {
                        await controller.deleteBeneficiary(beneficiary);
                      },
                    );
                  },
                );
...
class BeneficiaryTile extends StatefulWidget {
  ...
  @override
  _BeneficiaryTileState createState() => _BeneficiaryTileState();
}

class _BeneficiaryTileState extends State<BeneficiaryTile> {
              ...
              children: [
                IconButton(
                  ...
                  onPressed: () {
                    showGeneralDialog(
                      context: context,
                      barrierLabel: 'Editar beneficiario',
                      barrierDismissible: true,
                      barrierColor: Colors.black.withOpacity(0.5),
                      transitionDuration: const Duration(milliseconds: 300),
                      pageBuilder: (context, animation, secondaryAnimation) {
                        return BeneficiaryDetailPanel(beneficiary: widget.beneficiary);
                      },
                      transitionBuilder: (context, animation, secondaryAnimation, child) {
                        var slideAnimation = Tween<Offset>(
                          begin: const Offset(1, 0),
